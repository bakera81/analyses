...
# Load the proto file which specifies the format the feed files follow:
download.file(
  url = "https://api-endpoint.mta.info/Dataservice/mtagtfsfeeds/camsys%2Fsubway-alerts", 
  destfile = "nyc_transit/nyct_subway_alerts.proto")

url <- "https://api-endpoint.mta.info/Dataservice/mtagtfsfeeds/camsys%2Fsubway-alerts"
temp_file <- tempfile()

resp <- httr::GET("https://api-endpoint.mta.info/Dataservice/mtagtfsfeeds/camsys%2Fall-alerts")
...
RProtoBuf::descriptor()
ls("RProtoBuf:DescriptorPool")

              
read_gtfs_rt <- function(url, api_key = NULL) {
  ...
  if (httr::status_code(response) != 200) {
    stop("Failed to fetch GTFS-RT data: ", httr::http_status(response)$message)
  }
  ...
  feed <- protolite::parse_proto(binary_content)
