...
alerts_lookup <- alerts_clean %>%
  rename_with(
    ~paste0(., "_current_alert"),
    -any_of(c("route_id", "service"))) %>%
  inner_join(
    alert_durations %>%
      rename_with(
        ~paste0(., "_past_alerts"),
        -any_of(c("affected", "service"))), 
    by = c("route_id" = "affected", "service"),
    suffix = c("_current_alert", "_historical"),
...
alert_durations %>%
  ...
  group_by(date = floor_date(start_time, "week"), most_major_alert) %>%
  summarise(
    n_events = n(),
    mean_updates = mean(n_updates, na.rm = T)) %>%
  ggplot(aes(date, n_events, color = most_major_alert)) + 
  geom_line()
...
# Feature engineer: Create additional time-based features (hour of day, day of week, month) from timestamps
